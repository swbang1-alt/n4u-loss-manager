...
def find_columns(rows: list[tuple[Any, ...]]) -> tuple[int, dict[str, int]]:
    ...
    raise ValueError(
        "소분리스트에서 필수 열인 '창고명'과 '판매금액/금액'을 찾지 못했습니다."
    )
...
def process_files(
    ...
    for row in rows[header_index + 1:]:
        ...
        # 품목코드
        item_code = (
            normalize_code(row[columns["code"]])
            if "code" in columns
            else ""
        )
...
if records is not None:
    ...
    # =====================================================
    filtered = frame[
        (frame["거래일자"] >= start_date.isoformat())
        & (frame["거래일자"] <= end_date.isoformat())
    ]
